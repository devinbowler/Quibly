...
function Task() {
    ...
    const debouncedSaveNote = useCallback(
      debounce(async (note) => {
        ...
        try {
          ...
          // Display "Saved." for 2 seconds after saving
          setTimeout(() => {
    ...
    const handleCreate = async (type) => {
      ...
      if (type === 'folder') {
        ...
        // Note: you send type: 'file' but backend doesn’t return it.
    ...
    const handleCreateTask = async () => {
        const newTaskData = {
            title: newTask.name,  // Ensure this matches the schema's "title"
            details: newTask.details,  // Ensure this matches "details"
            dueDate: newTask.dueDate,  // Ensure this is a valid date
            parentFolder: currentPath,  // Ensure this is passed correctly
        };
        
        console.log('Creating task with data:', newTaskData);  // Log task data before sending it
        
        try {
            const createdTask = await createTask(newTaskData);
            console.log('Task created:', createdTask);  // Log the created task
            setTasks([...tasks, createdTask]);  // Add the task to the state
            setIsAddingTask(false);  // Close the modal after creating the task
            setIsEditingTask(false);
            setNewTask({ name: '', dueDate: '', details: '' });  // Reset the form
            setErrorMessage(null);
        } catch (error) {
    ...
    const handleUpdateTask = async () => {
      ...
      try {
        const updatedTask = await updateTask(selectedTask._id, {
          title: selectedTask.title, // assuming mapping from task.name to task.title
          dueDate: selectedTask.dueDate,
          details: selectedTask.details,
        });
        console.log('Task updated:', updatedTask);
        // Update the tasks state so that changes are immediately reflected
        setTasks(tasks.map(task => task._id === updatedTask._id ? updatedTask : task));
        // Close the modal and ensure the view is still showing tasks
    ...
    // Handle saving (updating) a note (not used since auto-save is implemented)
    ...
    const handleDelete = async () => {
      ...
      try {
        ...
        if (viewType === 'taskDetails') {
          ...
          // Remove the note from the files list immediately
    ...
    const openTaskDetailsModal = (task) => {
        ...
        setViewType('taskDetails'); // Change viewType to indicate task details
    };
    ...
    }, [isAddingTask, selectedTask, currentPath, searchQuery]); // Added dependencies
    ...
    return (
      <div className="app-container">
        ...
        <div className="top-bar">
          ...
          <span className="settings-gear" onClick={() => alert('Settings Clicked')}>
            ⚙️
          </span>
        </div>
        ...
        <div className="filter-buttons">
          ...
          <div className="right-buttons">
            {viewType === 'task' ? (
              ...
              // When in note view, the Save/Delete buttons will be shown inside the note view section.
              <></>
            ) : (
              <>
        ...
        {selectedTask && (
          <div className="modal-overlay">
            <div className="modal-view">
              {/* Title Input bound to selectedTask.title */}
              <textarea readonly
                className="modal-title"
                type="text"
                value={selectedTask.title}
              />
              {/* Date Input properly formatted as YYYY-MM-DD */}
              <textarea readonly
              ...
              <div className="modal-buttons">
                ...
                <button
                  ...
                  onClick={() => {
                    setSelectedTask(null);
                    setViewType('task'); // Keeps the task view visible when closing
                  }}
                >
                  Close
                </button>
